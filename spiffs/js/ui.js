...
export function updateStatusSectionUI() { if (uiElements.imuStatusValue) uiElements.imuStatusValue.textContent = imuStatusText(appState.currentSystemState.imu); const state = appState.currentSystemState; if (uiElements.systemStateValue) uiElements.systemStateValue.textContent = state.state_name || 'UNKNOWN'; if (uiElements.systemStateId) uiElements.systemStateId.textContent = (state.state_id !== undefined && state.state_id !== null) ? state.state_id : '?'; updateAutoBalancingButtonUI(); updateFallDetectButtonUI(); updateYawControlButtonUI(); updateCriticalBatteryShutdownButtonUI(); updateGuidedCalibrationUI(); updateOtaUI(); updatePidTuningUI(); const batt = appState.currentBattery; updateBatteryUI(batt.voltage, batt.percentage); }
