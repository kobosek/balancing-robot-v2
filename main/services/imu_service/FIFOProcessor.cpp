...
esp_err_t FIFOProcessor::disableFIFO(bool unregisterISR) {
    // First disable the FIFO (using the base method)
    ESP_LOGD(TAG, "Disabling FIFO with ISR unregistration option: %d", unregisterISR);
    esp_err_t ret = m_driver.disableFIFO();
    
    // If requested and ISR handler is installed, unregister it
    if (unregisterISR && m_isr_handler_installed && m_interrupt_pin >= 0) {
...
esp_err_t FIFOProcessor::resetAndReEnableFIFO() {
    ...
    // If ISR was previously installed, re-enable with interrupt registration
    if (isr_was_installed && interrupt_pin >= 0 && interrupt_pin < GPIO_NUM_MAX) {
        // We need to know if the pin was active high or low, but we can't store that easily
        // For now, we'll assume active high as default (most common case)
        bool active_high = true;
        ret = enableFIFO(interrupt_pin, active_high);
    } else {
        // Standard enable without ISR registration
        ret = enableFIFO();
    }
...
void FIFOProcessor::incrementIsrDataCounter() {
    ...
    uint8_t count = m_interrupt_counter.fetch_add(1, std::memory_order_relaxed) + 1;
    ...
    if (count >= INTERRUPTS_PER_PROCESS) {
        ...
        m_isr_data_counter.fetch_add(1, std::memory_order_relaxed);
        
        // Signal the semaphore to wake up the FIFO task
        if (m_dataReadySemaphore != NULL) {
            BaseType_t xHigherPriorityTaskWoken = pdFALSE;
            xSemaphoreGiveFromISR(m_dataReadySemaphore, &xHigherPriorityTaskWoken);
...
uint8_t FIFOProcessor::getAndResetIsrDataCounter() {
    return m_isr_data_counter.exchange(0, std::memory_order_relaxed);
}
